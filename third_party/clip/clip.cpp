...
void default_error_handler(ErrorCode code) {
  // picopaste local patch: the project builds without exceptions and with
  // error codes only. Upstream threw std::runtime_error here, which is the
  // only throw in the whole library. It is replaced by a no-op so a failed
  // clipboard lock surfaces as lock::locked() == false (the caller's error
  // path) instead of an exception. Everything else in the file is untouched.
  (void)code;
}
