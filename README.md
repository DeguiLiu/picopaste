...
flowchart LR
  ...
  TR["Tray icon"]:::win
  PP["picopaste.exe<br/>single process · Job Object 64 MB"]:::core
...
5. A large screenshot needs room in `job_memory_limit_mb`: Windows materialises the clipboard image
   inside picopaste's own process, so the default 64 MB covers 4K captures. If only big
   screenshots fail, that ceiling is the first thing to raise — an 8K capture needs roughly 132 MB.
