...
| 3 | 极致省资源 | 空闲 CPU 为 0（非"接近 0"）；稳态内存 ≤ 12 MB；硬上限 64 MB（须容纳 4K 截图取图时 Windows 拷进本进程的整张图；8K 需自行调高）；单进程 |
...
于是失败被显示成"剪贴板里没有图片"。默认上限因此是 64 MB，不是按稳态占用算出来的 32 MB：
消除那份合成拷贝之后，一张 4K 截图的真实峰值只需 33 MB 加上进程与 ssh 子进程的开销。
...
| **硬上限** | **64 MB** | Job Object 的内核强制 commit 上限。它不是稳态占用，而是要覆盖"取图时整张图被复制进本进程"这一瞬时峰值；空闲与建链后的实测值仍是上面的 12 MB / 40 MB |
...
| `job_memory_limit_mb` | `64` | 内核强制内存上限（MB）。须大于"一整张图被复制进本进程"的峰值，否则大截图取不到。默认覆盖 4K（约 33 MB）；8K（约 132 MB）须自行调高 |
