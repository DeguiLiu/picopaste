// picopaste -- Win32 clipboard capture to a single temporary file.
//
...
// Pure format probe. Does not read pixels and does not create a temp file.
ClipboardFormats InspectClipboardFormats() noexcept;

// Capture the clipboard image to one temp file. The clipboard is closed before
// return. Returns kNoImageInClipboard when the clipboard holds no image;
// kImageTooLarge when the encoded image would exceed cfg.max_image_bytes;
...
// Encode a raw DIB (BITMAPINFOHEADER, BITMAPV4HEADER or BITMAPV5HEADER followed
// by palette and pixels) to `out_path` as PNG. Exposed so selftest can exercise
// the encoder with a synthetic DIB without a real clipboard. `dib` must stay
// valid for the duration of the call; it is read in place, never copied.
...
// Delete the temp file behind `image`. Safe to call on a default-constructed
// value and safe to call twice.
