// picopaste -- Windows platform layer shared helpers.
//
...
inline bool Utf8ToWide(const char* in, wchar_t* out, std::size_t out_chars) noexcept {
  ...
  int written = MultiByteToWideChar(CP_UTF8, 0, in, -1, out, static_cast<int>(out_chars));
  return written > 0;
}
...
inline bool WideToUtf8(const wchar_t* in, char* out, std::size_t out_bytes) noexcept {
  ...
  int written =
      WideCharToMultiByte(CP_UTF8, 0, in, -1, out, static_cast<int>(out_bytes), nullptr, nullptr);
  return written > 0;
}
